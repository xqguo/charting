...
Plotly need to summarize the data before plotting while Seaborn has built-in stats function and distribution allows plot a distribution in charts
...
Here is the seaborn chart. A lot less code needed, and more features in the chart.

```{python}
fig = sns.lineplot(gapminder, x="year", 
              y="lifeExp", 
              hue="continent",               
              )
plt.show()
...
Seaborn bar plot automatically shows a clustered bar with built-in stats for average and error bar, but creating a stacked bar seems difficult.
...
Plotly needs to manually compute stats and no error bar. but otherwise shows a clusters bar easily
...
### Pair plot
Seaborn built-in stats make the a lot better than plotly
...
Plotly scatter matrix is scatter only.
...
### Plotly has nice type of charts like sunburst and geo plots

A sunburst chart shows hierarchy and hover info is very useful here.
...
A geo plot using iso_alpha codes and plot population. 
...
Plot routes in the earch surface
...
### Plotly can have interactivity and animation
